...
class DecodeActivity : ComponentActivity() {
        ...
        when (intent.action) {
            ...
            Intent.ACTION_SEND_MULTIPLE -> {
                intent.getParcelableArrayListExtra<Uri>(Intent.EXTRA_STREAM)?.let { uris ->
                    multi = uris
                    Toast.makeText(this, "Files: ${uris.size}", Toast.LENGTH_SHORT).show()
                }
            }
        }
        setContent {
    ...
    private fun getEncodedFileUri(fileName: String): Uri? {
        ...
        val selectionArgs = arrayOf(fileName.removeSuffix(".reverse"))
        val cursor =
        ...
        return if (cursor?.moveToFirst() == true) {
            ...
            val values =
                ContentValues().apply {
                    put(MediaStore.MediaColumns.DISPLAY_NAME, fileName.removeSuffix(".reverse"))
    ...
    fun DecodeScreen() {
        ...
        var status by remember { mutableStateOf("Decoding...") }
        var detail by remember { mutableStateOf("") }
        var enable by remember {
            mutableStateOf(false)
        }
        Column {
            Text(text = stringResource(R.string.decode_title), fontStyle = MaterialTheme.typography.headlineSmall.fontStyle)
            Text(
                detail.let {
                    if (it.isNotBlank()) {
                        it + "\n"
                ...
                fileUri?.let {
                    try {
                        ...
                        if (inputStream != null && outputStream != null) {
                            if (!Core.isMagicMatch(Core.readMagic(inputStream))) {
                                status = getString(R.string.meta_mismatch)
                                return@launch
                            }
                            val meta = Core.readMeta(inputStream)
                            val total = getFileSize(applicationContext, it) - 558
                            detail = meta.toString() + "\nSize:" + total + "(meta size:" + 558 + ")"
                            var sum = 0
                            var i = 0
                            Core.reverse(inputStream, outputStream, meta.blockSize.toInt()) {
                                ...
                                if (i++ % 10 == 0) {
                                    status =
                                        getString(R.string.reversing_5_2f_d_d).format(
                            ...
                            status += getString(R.string.file_recovered_d_d).format(sum, total)
                            inputStream.close()
                            outputStream.flush()
                            outputStream.close()
                            enable = true
    ...
    private fun getDecodedOutputStream(_uri: Uri): OutputStream? {
        val fileName = getFileName(_uri).removeSuffix(".reverse")
        Log.d("Decode", "filename: " + fileName)
